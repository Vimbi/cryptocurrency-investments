...
import { EtherScanService } from '../../ether-scan/ether-scan.service';
import { IEtherTransactionInfo } from '../../../types/ether-scan/ether-transaction-info.interface';
...
import { ITransferScanEther } from '../../../types/transfers/transfer-scan.interfaces';

@Processor(BlockChainExplorerEnum.etherscan)
export class EtherscanTransferConsumer {
  ...
  constructor(
    ...
    this._minConfirmationsLimit = this.configService.get(
      'ether-scan.minConfirmationsLimit',
    );
  }
  ...
  async transferScan({ data }: Job<{ transferId: string }>) {
    ...
    try {
      await depositTransferProcessingInQueueForScanning({
        manager,
        transferId,
        networkTokenType: NetworkTokenTypeEnum.erc20,
      ...
      this.logger.error(`${errorMsgs.queueError(
        BlockChainExplorerEnum.etherscan,
      )}
  ...
  private async _scan({
    ...
    const scanResult = await this.etherScanService.getTransaction({
      hash: txId,
  ...
  private _scanResultValidation({
    ...
    scanResult: IEtherTransactionInfo;
  ...
  async handleError(job: Job) {
    this.logger.error(`Queue ${BlockChainExplorerEnum.etherscan} ERROR.
